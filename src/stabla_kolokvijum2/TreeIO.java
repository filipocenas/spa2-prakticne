...
public class TreeIO<T> {
	...
	protected void appendRight(StringBuilder builder, Object tree, Config config, String[] buildingBlocks,
			boolean isRight, String prefix) {
		if (tree == null) {
			return;
		}
	...
	protected void appendLeft(StringBuilder builder, Object tree, Config config, String[] buildingBlocks,
			boolean isRight, String prefix) {
		if (tree == null) {
			return;
		}
	...
	protected void appendSubtree(StringBuilder builder, Object tree, Config config, String[] buildingBlocks,
			boolean isRight, String prefix) {
	...
	public TreeIO(Class<T> type, Config config) {
		...
		if (declaredClasses.length == 0) {
			throw new IllegalArgumentException(
					"Klasa " + this.treeType.getName() + " nema unutrasnju klasu koja predstavlja cvorove");
		}
		...
		if (staticOne == null) {
			throw new IllegalArgumentException(
					"Klasa " + this.treeType.getName() + " nema staticku unutrasnju klasu koja predstavlja cvorove");
		}
		if (multiStatic) {
			throw new IllegalArgumentException("Klasa " + this.treeType.getName()
					+ " ima vise unutrasnjih statickih klasa, a mora biti samo jedna");
		}
		...
		for (Constructor<?> constructor : declaredConstructors) {
			...
			if (parameters.length == 0 && !throwingExceptions) {
				defaultTreeConstructor = (Constructor<T>) constructor;
			}
			if (parameters.length == 1 && parameters[0].isAssignableFrom(this.nodeType) && !throwingExceptions) {
				treeConstructor = (Constructor<T>) constructor;
			}
		}
		if (defaultTreeConstructor == null && treeConstructor == null) {
			throw new IllegalArgumentException(
					"Klasa " + this.treeType.getName() + " nema " + this.nodeType.getSimpleName() + "() ili "
		...
		for (Field field : declaredFields) {
			...
			if (field.getType().isAssignableFrom(this.nodeType)) {
				if (rootField != null) {
					throw new IllegalArgumentException("Klasa " + this.treeType.getName()
							+ " ima vise polja koji bi mogli predstavljati koren stabla");
				}
				rootField = field;
			}
		}
		if (rootField == null) {
			throw new IllegalArgumentException(
					"Klasa " + this.treeType.getName() + " nema polje za predstavljanje korena");
		}
		this.rootField = rootField;
		if (Modifier.isStatic(this.rootField.getModifiers())) {
			throw new IllegalArgumentException(
					"Polje " + this.treeType.getName() + "." + this.rootField.getName() + " ne sme da bude staticko");
		}
		...
		if (declaredFields.length == 0) {
			throw new IllegalArgumentException(
					"Unutrasnja klasa " + this.nodeType.getName() + " nema deklarisanih polja");
		}
		...
		for (Field field : declaredFields) {
			...
			if (fieldName.startsWith("e") || fieldName.startsWith("i") || fieldName.startsWith("o")) {
				if (elementField != null) {
					throw new IllegalArgumentException("Unutrasnja klasa " + this.nodeType.getName()
							+ " ima vise polja za predstavljanje elementa");
				}
				elementField = field;
			}
			if (fieldName.startsWith("l") && field.getType().isAssignableFrom(this.nodeType)) {
				if (leftField != null) {
					throw new IllegalArgumentException("Unutrasnja klasa " + this.nodeType.getName()
							+ " ima vise polja za predstavljanje levog podstabla");
				}
				leftField = field;
			}
			if (fieldName.startsWith("d")
					|| fieldName.startsWith("r") && field.getType().isAssignableFrom(this.nodeType)) {
				if (rightField != null) {
					throw new IllegalArgumentException("Unutrasnja klasa " + this.nodeType.getName()
							+ " ima vise polja za predstavljanje desnog podstabla");
				}
				rightField = field;
			}
		}
		if (elementField == null) {
			throw new IllegalArgumentException(
					"Unutrasnja klasa " + this.nodeType.getName() + " nema polje za predstavljanje elementa");
		}
		if (leftField == null) {
			throw new IllegalArgumentException(
					"Unutrasnja klasa " + this.nodeType.getName() + " nema polje za predstavljanje levog podstabla");
		}
		if (rightField == null) {
			throw new IllegalArgumentException(
					"Unutrasnja klasa " + this.nodeType.getName() + " nema polje za predstavljanje desnog podstabla");
		}
		this.elementField = elementField;
		if (Modifier.isStatic(this.elementField.getModifiers())) {
			throw new IllegalArgumentException("Polje " + this.treeType.getName() + "." + this.elementField.getName()
					+ " ne sme da bude staticko");
		}
		...
		if (Modifier.isStatic(this.leftField.getModifiers())) {
			throw new IllegalArgumentException(
					"Polje " + this.treeType.getName() + "." + this.leftField.getName() + " ne sme da bude staticko");
		}
		this.leftField.setAccessible(true);
		this.rightField = rightField;
		if (Modifier.isStatic(this.rightField.getModifiers())) {
			throw new IllegalArgumentException(
					"Polje " + this.treeType.getName() + "." + this.rightField.getName() + " ne sme da bude staticko");
		}
		...
		for (Constructor<?> constructor : declaredConstructors) {
			...
			if (parameters.length == 1 && parameters[0].isAssignableFrom(this.elementType) && !throwingExceptions) {
				nodeConstructor = constructor;
			}
			if (parameters.length == 3 && parameters[0].isAssignableFrom(this.elementType)
					&& parameters[1].isAssignableFrom(this.nodeType) && parameters[2].isAssignableFrom(this.nodeType)
					&& !throwingExceptions) {
		...
		if (defaultNodeConstructor == null && nodeConstructor == null && nodeConstructor3 == null) {
			throw new IllegalArgumentException(
					"Unutrasnja klasa " + this.nodeType.getName() + " nema " + this.nodeType.getSimpleName() + "() ili "
							+ this.nodeType.getSimpleName() + "(" + this.elementType.getName() + ") ili "
							+ this.nodeType.getSimpleName() + "(" + this.elementType.getName() + ", "
							+ this.nodeType.getSimpleName() + ", " + this.nodeType.getSimpleName() + ") konstruktor");
		}
		...
		for (Method method : declaredMethods) {
			...
			boolean familiarName = methodName.equals("fromString") || methodName.equals("valueOf")
					|| methodName.equals("parse" + this.elementType.getSimpleName());
			boolean goodParameters = method.getParameterTypes().length == 1
					&& (method.getParameterTypes()[0] == String.class || method.getParameterTypes()[0] == Object.class);
			if (familiarName && goodParameters && this.elementType.isAssignableFrom(method.getReturnType())
					&& !throwingExceptions) {
				if (elementFactoryMethod != null) {
					throw new IllegalArgumentException(
							"Klasa " + this.elementType.getName() + " ima vise " + this.elementType.getSimpleName()
									+ " fromString(String), " + this.elementType.getSimpleName() + " valueOf(String) i "
									+ this.elementType.getSimpleName() + " parse" + this.elementType.getSimpleName()
									+ "(String) metoda");
				}
		...
		if (elementFactoryMethod == null) {
			throw new IllegalArgumentException("Klasa " + this.elementType.getName() + " nema "
					+ this.elementType.getSimpleName() + " fromString(String), " + this.elementType.getSimpleName()
					+ " valueOf(String) ili " + this.elementType.getSimpleName() + " parse"
					+ this.elementType.getSimpleName() + "(String) metod");
		}
